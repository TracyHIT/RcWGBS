...
#' @param cov_cutoff_low the cov count used to select the site,>
#' @param cov_cutoff_high the cov count used to select the site,<
#'
#' @return A list containing Flank_methyl,Flank_DNA_onehot,Flank_DNA_2mer,Center_loci_table,Flank_m and Flank_um)
#' @export
#'
#' @examples
make_tain_Rdata <- function(cov_filename = "data/Methyl/ENCFF003FWN.0.5.deduplicated.bismark.cov.gz", flank_size = 50,
                                 output_prefix = "data/Train_Test_Data/ENCFF003FWN.0.5", train_size =100000,ref_genome_dir,file_type = ".gz",cov_cutoff_low=-1,cov_cutoff_high=-1)
{
  ...
  {train_methylation_start <- cov_cutoff_low}
  if(cov_cutoff_high == -1)
  {
    train_methylation_end <- mean(cov) + 3 * sd(cov)
  }else
  {
    train_methylation_end<-cov_cutoff_high
  }
  ...
  print(paste0("Here ",length(train_index_all)," sites can be used to train model."))
  if(ceiling(train_size * 1.2) > length(train_index_all))
  {
    print(paste0("As train_size is ",train_size,", so we do not need to do sample."))
    train_index_all_index <- train_index_all
    train_size <- length(train_index_all)
  }else
  {
                                             ...
                                             cov_high_threshold=1000)
{
  ...
  for(temp_chr in c(paste0("chr", c(1:22)), "chrX", "chrY"))
  {
    ...
    Flank_DNA_2mer <- list()#@@@@@@@
    Flank_DNA_onehot[[length(test_index_all_index)]] <- 0
    Flank_DNA_2mer[[length(test_index_all_index)]] <- 0
    ...
    for (i in 1:length(test_index_all_index)) {
      if(length(Flank_DNA_2mer[[i]])==0 | length(Flank_DNA_2mer[[i]])==1)
      {
        out_filt <- c(out_filt, i)
      }else{
